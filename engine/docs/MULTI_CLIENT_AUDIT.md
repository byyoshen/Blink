# Blink Multi-Client Audit（Surge / Shadowrocket / Loon / Stash / Egern / Quantumult X / mihomo）
...
### mihomo（Clash Meta for Android / FLClash 通用；官方文档 + 内核源码 + 生产实证）
...
- **Blink 7 种 canonical 类型：6/7 无损，USER-AGENT 为唯一降级项**（受影响 4 条：Netflix / ParamountPlus / PayPal / Spotify 各 1）→ 独立 `mihomo/` 目录**显式丢弃并计数**（与 Egern/QX 对 PROCESS-NAME 的先例一致；2026-08 定稿）。
...
- 输出架构决策：`mihomo/<App>.list` = classical body 去掉 USER-AGENT 行（其余 6 类逐行同 Surge）；渲染器 = classical 加 per-target 丢弃集；Surge 字节不变（backward compat 门禁不变）。

## 3. Client Capability Matrix

| Rule Type | Surge | Shadowrocket | Loon | Stash | Egern | Quantumult X | mihomo |
|---|---|---|---|---|---|---|---|
...
| | Surge | Shadowrocket | Loon | Stash | Egern | Quantumult X | mihomo |
|---|---|---|---|---|---|---|---|
...
| 独立 classical 文件 | ✅ | ✅ | ✅ | ✅（format: text） | ✅ | ❌ 需自有 HOST* filter 格式 | ✅（format: text，独立 mihomo 目录去 UA） |
...
- **A. 可共享同一 output**：Surge / Shadowrocket / Loon / Stash 共享**逐字节相同**的 classical `.list`；mihomo 为第 5 个 classical 消费端，但内容 = classical 去掉 USER-AGENT（内核无此类型），故**不**共享逐字节文件、独立 `mihomo/` 目录输出。
- **B. 引用方式不同、Rule-Set 内容相同**：上述四客户端；Egern 也能直接消费同一 classical 文件（`rule_set.match`）；mihomo 经 `rule-providers`（`behavior: classical, format: text`）消费 UA 丢弃后的 classical 变体。
- **C. 真正需要不同 serialization**：**Egern 自有 YAML schema**（可选但采纳）与 **Quantumult X 自有 filter 格式**（必需：QX 不消费 policy-free classical）。共 3 种序列化格式覆盖 7 客户端（mihomo 复用 classical 序列化 + per-target 丢弃集）。
- **D. 无法无损表达的 Canonical Type**：对 **Egern ❌、Quantumult X ❌** 只有 **PROCESS-NAME**；对 **mihomo ❌** 是 **USER-AGENT**（内核无此类型，classical 加载器静默跳过 → 必须显式丢弃）。Loon 与 Shadowrocket 无 PROCESS-NAME 类型，但 §7 classical 渲染器的逐字节一致性要求保留该行（客户端忽略未知规则类型），不属于序列化降级。QX 另外无法表达 no-resolve 选项（生产源全部省略）。
- **E. 必须 capability fail/downgrade 的项目**：PROCESS-NAME → 对 Egern / Quantumult X **显式降级丢弃**（构建报告计数，绝不 silent）。受影响 App：Netflix（1 条）、Spotify（1 条）、Disney（2 条）、Hulu（1 条）、Twitch（1 条）、HBO（1 条）。USER-AGENT → 对 mihomo **显式降级丢弃**（构建报告计数），受影响 App：Netflix（1 条）、ParamountPlus（1 条）、PayPal（1 条）、Spotify（1 条）。Egern 的 `no_resolve: true` 为 set 级：全部 IP 规则带 no-resolve 时输出，全不带时省略（语义等价），**混合时显式构建失败**。QX 的 no-resolve 省略为全类型统一行为，记入本文档而非逐行报告。

## 6. Shared Output Feasibility

**可行且为最优解**。共享 classical 输出 ×4 客户端 + mihomo classical 变体（去 USER-AGENT）×1 + Egern YAML ×1 + QX filter ×1，共 **3 种序列化格式覆盖 7 客户端**。Stash 与 mihomo 的关键疑问已有答案：classical text 直接消费（`format: text`），**不需要为它们制造 YAML Rule-Set**；mihomo 唯一差异是内核无 USER-AGENT → 独立目录显式丢弃并计数。
...
| `classical-mihomo` | classical 变体（去 USER-AGENT，其余 6 类逐行同 Surge） | mihomo | 6 类保留；USER-AGENT → 显式丢弃 + 报告计数 |
...
mihomo/<App>.list       # classical 去 USER-AGENT（其余逐行同 Surge；UA 丢弃计数）
...
9. mihomo 端 `keep-alive-interval` / `unified-delay` 等 Meta 扩展字段在 CMFA / FLClash 的实际表现（官方文档支持，真机验证）。
...
- **B · YouTube 跨端 E2E**：当前主源 Repcz 14 条不变；七端最小引用示例内置 README（mihomo 示例含 UA 丢弃说明）；真机验证首页/搜索/播放/Shorts/图床/CDN/评论/API，观察落 Final、CDN 漏网与七端一致性。
...
- 2026-08：第 7 客户端 **Clash（Mihomo 内核 / Android 通用）**审计落地（见 §2.8，内核源码级证据）；`mihomo/` 目录（去 USER-AGENT 显式丢弃）与 portal / Profiles / 文档同步扩展中。
- 2026-09-13：第 7 个客户端由 **Clash 重命名为 mihomo**（client key、`Clash/` → `mihomo/`、
  `Profiles/Clash.yaml` → `Profiles/mihomo.yaml`、renderer `classical-clash` → `classical-mihomo`）。
  本仓库 target 一直是 mihomo 内核（见 §2 该章节），「Clash」只是遗留的显示名；上游 Repcz/Tool 同样以
  `mihomo/` 命名。关于 Clash 系内核特性的表述（无 USER-AGENT 类型等）未变 —— mihomo 属于该族。
  **破坏性变更**：旧 `Clash/*.list` raw URL 失效。
- 2026-08-21+：**语义多视图落地**（SukkaW 式 domainset / non_ip / ip 拆分，见 `build.py` 的 `semantic_views` / `phase_of` 与 `engine/scripts/validate_views.py` 门禁）：每个 App 按语义派生出 `-domainset.conf`（纯域名，Surge/Shadowrocket 域名清单、Stash/mihomo behavior:domain）/ `-nonip.conf`（含 keyword/UA/PROCESS，classical）/ `-ip.conf`（IP 段，classical/QX filter）七端视图，IP 段恒置于域名段之后；主输出 `.list`/`.yaml` 与各客户端格式事实保持本文件所述不变。
...
### 13.1 为什么另立目标，而不是替换 mihomo

维护者最初的提议是用 sing-box 顶替该目标（当时名为 Clash），理由是「mihomo 内核的配置文件通用」。前半句成立 —— mihomo YAML 在 CMFA / FLClash 上确实通吃；但**规则集文件不在此列**。

`mihomo/` 目录独立存在的唯一理由（见 §5 D/E）是：**Clash 系内核没有 `USER-AGENT` 规则类型**，classical 加载器遇到会 warning 后静默跳过，所以该目录 = classical 去 UA。删掉它，mihomo 用户只能改用 `Stash/` 的文件，而那里**带着他们内核看不懂的规则行** —— 这不是少了便利，是引入静默失效，与 F1 / F15 同类。

外加该目录是 68 个已发布的稳定 raw URL，移除会直接打断现有引用。
...
- **USER-AGENT → 显式丢弃 + 计数**，与 mihomo / Egern / QX 同一处置方式，绝不静默。
